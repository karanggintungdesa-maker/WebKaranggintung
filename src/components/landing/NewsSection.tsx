...
export function NewsSection() {
  ...
  return (
    <section className="relative mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8 lg:py-28 overflow-hidden">
      {/* Section Header */}
      <motion.div
      ...
      >
        <div className="max-w-2xl space-y-3">
          <div className="inline-flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-4 py-1.5 text-xs font-bold uppercase tracking-widest text-emerald-700">
        ...
        <Link
          href="/BeritaDesa/"
          className="inline-flex items-center gap-2 text-sm font-bold text-emerald-700 hover:text-emerald-800 transition-colors shrink-0"
        >
          Lihat Semua Berita
      ...
      {isLoading ? (
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 3 }).map((_, index) => (
            <div key={index} className="rounded-[2rem] border border-slate-100 bg-white p-4 shadow-sm">
        ...
        <div className="mt-12 rounded-[2rem] bg-amber-50 p-6 text-sm text-amber-700 border border-amber-200">
          Berita sedang tidak dapat dimuat saat ini. Silakan kunjungi halaman Berita Desa secara langsung.
        </div>
      ) : !news || news.length === 0 ? (
        <div className="mt-12 rounded-[2rem] bg-slate-50 p-8 text-center text-sm text-slate-600 border border-slate-100">
        ...
        <div className="mt-12 relative w-full overflow-hidden marquee-gradient-mask">
          <div className="animate-marquee-slow flex gap-6 py-4">
            {duplicatedNews.map((item, idx) => (
              <motion.article
                key={`${item.id}-${idx}`}
                className="w-[340px] sm:w-[380px] shrink-0 group overflow-hidden rounded-[2.25rem] border border-slate-200/80 bg-white/95 backdrop-blur-sm shadow-md hover:shadow-2xl transition-all duration-300 flex flex-col justify-between"
                whileHover={{ y: -6 }}
              >
                <div>
                  ...
                  <div className="p-6">
                    ...
                    <h3 className="mt-3 text-lg font-black text-slate-900 line-clamp-2 h-14 leading-tight group-hover:text-emerald-800 transition-colors font-display">
                      {item.title}
                    </h3>
                    ...
                    <ul className="mt-4 space-y-1.5 pt-2 text-xs font-semibold text-slate-600 border-t border-slate-100">
                      <li className="flex items-center gap-2">
                        <CheckCircle2 className="h-3.5 w-3.5 text-emerald-600 shrink-0" />
                        <span>Kabar Resmi Pemdes Karanggintung</span>
                      </li>
                    </ul>
                  </div>
                </div>
                ...
                <div className="px-6 pb-6 pt-0">
                  <Link
                    ...
                    className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-emerald-700 hover:text-emerald-800 transition-colors"
                  >
                    Baca Selengkapnya
      ...
      <motion.div
        ...
        className="mt-20 overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-950 text-white p-6 sm:p-10 lg:p-12 shadow-2xl border border-emerald-900/30"
      >
        ...
        <div className="overflow-hidden rounded-[2rem] aspect-video w-full bg-slate-900 shadow-2xl border border-white/10">
          {youtubeEmbedUrl ? (
            <iframe
              src={youtubeEmbedUrl}
            ...
            <div className="flex h-full min-h-[320px] items-center justify-center bg-slate-950 text-center text-slate-200">
              <div className="space-y-3 px-6">
                <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-emerald-600/20 border border-emerald-500/30 text-emerald-400">
                  <Play className="h-6 w-6 fill-current" />
                </div>
                <p className="text-base font-bold">Video profil desa dapat dikonfigurasi melalui Admin</p>
